...
class LegoWorldRuntime {
  constructor(renderer, scene, camera) {
    ...
    this.pitch = -0.08;
    this.velocityY = 0;
    this.player = new THREE.Vector3(-11.5, FLOOR_Y, 15.5);
    ...
    this.chunkRoot.name = 'Procedural chunks';
  ...
  #installStyles() {
    ...
    style.textContent = `
      ...
      #worldTouch .pad{grid-template-columns:52px 52px 52px;grid-template-areas:'. up .' 'left down right'}
      #worldTouch button{width:52px;height:52px;border:1px solid rgba(255,255,255,.25);border-radius:14px;background:rgba(10,18,12,.72);color:#fff;font:700 20px system-ui;backdrop-filter:blur(8px);touch-action:none}
      ...
      #worldTouch .actions{grid-template-columns:58px 58px}#worldTouch .actions button{width:58px;height:52px;font-size:13px}
      ...
      @media (pointer:coarse),(max-width:800px){body.world-explore #worldTouch{display:flex}#worldStats{bottom:82px}.hud{pointer-events:auto}}
    `;
  ...
  #installUI() {
    ...
    if (controls) {
      ...
      controls.prepend(this.playButton, this.viewButton, this.placeButton, this.removeButton, this.multiplayerButton);
    }
  ...
  #installInput() {
    ...
    for (const button of this.touch.querySelectorAll('[data-key]')) {
      ...
      const down = event => { event.preventDefault(); this.virtualKeys.add(key); };
      const up = event => { event.preventDefault(); this.virtualKeys.delete(key); };
      button.addEventListener('pointerdown', down);
      button.addEventListener('pointerup', up);
      button.addEventListener('pointercancel', up);
      button.addEventListener('pointerleave', up);
    }
    this.touch.querySelector('[data-action="jump"]').addEventListener('pointerdown', event => { event.preventDefault(); this.#jump(); });
    this.touch.querySelector('[data-action="place"]').addEventListener('pointerdown', event => { event.preventDefault(); this.#placeBrick(); });
    ...
    this.canvas.addEventListener('pointermove', event => {
      ...
      lastY = event.clientY;
      this.yaw -= dx * 0.008;
      this.pitch = THREE.MathUtils.clamp(this.pitch - dy * 0.006, -1.15, 1.0);
    });
  ...
  #jump() {
    if (!this.explore || !this.lastGrounded) return;
    this.velocityY = 5.4;
  ...
  #updateMovement(dt) {
    ...
    const speed = this.#pressed('ShiftLeft', 'ShiftRight') ? 7.4 : 4.5;
    ...
    if (moving) {
      ...
      this.player.addScaledVector(direction, speed * dt);
      this.stepPhase += dt * speed * 2.3;
    }
    ...
    this.player.x = THREE.MathUtils.clamp(this.player.x, -420, 420);
    this.player.z = THREE.MathUtils.clamp(this.player.z, -420, 420);
    this.#updatePlayerAvatar(moving ? speed : 0);
  }

  #updatePlayerAvatar(speed) {
    const bob = speed ? Math.sin(this.stepPhase * 2) * 0.025 : 0;
    ...
    const swing = speed ? Math.sin(this.stepPhase) * 0.55 : 0;
  ...
  #updateCamera() {
    ...
    if (this.view === 'first') {
      ...
      const offset = new THREE.Vector3(1.25, 0.72, 4.15).applyAxisAngle(new THREE.Vector3(0, 1, 0), this.yaw);
  ...
  #makeTree(x, z, scale, random) {
    const group = new THREE.Group();
    const trunk = new THREE.Mesh(new THREE.CylinderGeometry(0.2 * scale, 0.28 * scale, 3.4 * scale, 7), makeMaterial(0x67462b, 0.92));
    trunk.position.y = 1.45 * scale;
    trunk.castShadow = true;
    group.add(trunk);
    const leafMaterial = makeMaterial(0x245c31, 0.86);
    for (let i = 0; i < 5; i += 1) {
      const leaf = new THREE.Mesh(new THREE.BoxGeometry(0.28 * scale, 0.12 * scale, 2.5 * scale), leafMaterial);
      leaf.position.y = 3.25 * scale;
      leaf.rotation.y = i * Math.PI * 2 / 5 + random() * 0.2;
      leaf.rotation.x = -0.2;
      leaf.translateZ(0.95 * scale);
  ...
  #createChunk(cx, cz) {
    ...
    const baseColor = random() > 0.45 ? 0x3f7b3d : 0x4d8440;
    const base = new THREE.Mesh(new THREE.BoxGeometry(CHUNK_SIZE, 0.34, CHUNK_SIZE), makeMaterial(baseColor, 0.9));
    base.position.y = -0.06;
    base.receiveShadow = true;
    base.userData.worldGround = true;
    group.add(base);

    const studGeometry = new THREE.CylinderGeometry(0.18, 0.18, 0.09, 8);
    const studs = new THREE.InstancedMesh(studGeometry, makeMaterial(baseColor, 0.82), 100);
    studs.userData.worldGround = true;
    const matrix = new THREE.Matrix4();
    let index = 0;
    for (let z = 0; z < 10; z += 1) {
      for (let x = 0; x < 10; x += 1) {
        matrix.makeTranslation(-10.8 + x * 2.4, 0.16, -10.8 + z * 2.4);
    ...
    const distanceFromSet = Math.hypot(worldX, worldZ);
    if (distanceFromSet > 26) {
      const count = 3 + Math.floor(random() * 5);
      for (let i = 0; i < count; i += 1) {
        const x = (random() - 0.5) * (CHUNK_SIZE - 4);
        const z = (random() - 0.5) * (CHUNK_SIZE - 4);
        group.add(this.#makeTree(x, z, 0.65 + random() * 0.65, random));
      }
      for (let i = 0; i < 4; i += 1) {
        const bush = new THREE.Mesh(new THREE.IcosahedronGeometry(0.5 + random() * 0.5, 0), makeMaterial(0x2e6836, 0.9));
        bush.position.set((random() - 0.5) * 20, 0.62, (random() - 0.5) * 20);
        bush.scale.y = 0.7;
  ...
  #updateChunks(force = false) {
    ...
    if (!force && now - this.lastChunkUpdate < 500) return;
  ...
  #raycastScene() {
    ...
    return hits.find(hit => {
      ...
      return !isDescendantOf(object, this.playerAvatar)
        ...
        && !object.userData?.worldDecoration;
    });
  }
  ...
  #placeBrick() {
    ...
    const data = {
      ...
      x: snap(point.x),
      y: Math.max(FLOOR_Y + BUILD_HEIGHT * 0.5, Math.round(point.y / BUILD_HEIGHT) * BUILD_HEIGHT),
      z: snap(point.z),
      color: 0xc6944d,
    };
  ...
  #removeBrick() {
    ...
    const id = hit?.object?.userData?.buildId;
  ...
  #updateStats() {
    ...
    this.stats.textContent = `${this.explore ? 'PLAY' : 'SIM'} · ${this.peers.size + 1} player${this.peers.size ? 's' : ''} · ${this.builds.size} builds · ${x}, ${z}`;
  }
